...
Из расчётов выше для отдельных ручек, характеристики машин для запуска сервиса `scooter-rental-service`:
...
- 11 CPU из расчёта:
  ...
  - Исходя из суммарного RPS в 5300 и суммарного трафика с БД на чтение / запись в 5.4 MB / s потребуется хотя бы 11 CPU
- 1 GB оперативной памяти из расчёта:
  - Суммарный inflight запросов будет 320, на каждый запрос будем держать в памяти информацию о заказе (1 KB) и метаинформацию по запросу (оценим в 1 KB), получается 0.62 MB памяти
...
Из расчётов выше для отдельных таблиц, характеристики кластера БД с учётом `x10` запаса по ресурсам:
...
- 20 CPU, так как:
  ...
  - Суммарный PRS для базы будет 5300, что требует не менее одного CPU
  - Суммарная скорость чтения и записи будет хотя бы 5.4 MB / s, что требует не менее одного CPU
- 10 GB оперативной памяти, так как:
  - Оценим дополнительное потребление памяти на 1 inflight запрос, как размер строки в базе (1 KB) + дополнительные расходы в PG на один запрос как 1 KB, при суммарном inflight в 430 выходит будет нужно не менее чем 0.86 MB памяти
